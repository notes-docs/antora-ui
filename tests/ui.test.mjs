...
test('Lucode Antora UI templates contain the new shell markers', () => {
  ...
  const nav = readFileSync(join(srcDir, 'partials', 'nav.hbs'), 'utf8')
  ...
  const navCss = readFileSync(join(srcDir, 'css', 'nav.css'), 'utf8')
  ...
  const buildTask = readFileSync(join(packageDir, 'gulp.d', 'tasks', 'build.js'), 'utf8')
  const headScripts = readFileSync(join(srcDir, 'partials', 'head-scripts.hbs'), 'utf8')
  ...
  assert.match(header, /mobile-context-link/)
  assert.match(body, /site-shell/)
  assert.match(body, /<aside class="docs-toc">/)
  assert.match(main, /docs-main-inner/)
  assert.doesNotMatch(main, /<aside class="docs-toc">/)
  assert.match(nav, /mobile-global-nav/)
  assert.match(nav, /mobile-global-link/)
  ...
  assert.match(varsCss, /--toc-width:\s*24rem/)
  ...
  assert.match(tocCss, /max-height:\s*var\(--toc-height\)/)
  assert.match(tocCss, /overflow:\s*auto/)
  assert.match(tocCss, /width:\s*100%/)
  assert.match(tocCss, /min-width:\s*0/)
  assert.match(tocCss, /padding-right:\s*calc\(var\(--spacing\) \* 2\)/)
  assert.match(tocCss, /\.toc a/)
  assert.match(tocCss, /display:\s*block/)
  assert.match(tocCss, /text-wrap:\s*pretty/)
  assert.match(tocCss, /word-break:\s*break-word/)
  assert.match(tocCss, /li \+ li/)
  ...
  assert.match(navCss, /\.mobile-global-nav/)
  assert.match(navCss, /\.mobile-global-link/)
  assert.match(navCss, /@media screen and \(min-width:\s*1024px\)/)
  ...
  assert.match(themeJs, /try \{/)
  assert.match(themeJs, /media\.addListener/)
  assert.match(headScripts, /try \{/)
  assert.match(headScripts, /localStorage\.getItem\('lucode-theme'\)/)
  assert.match(buildTask, /postcssVar\(\{ preserve: true \}\)/)
})
